...
<%
   ...
   //会员首页显示时间改为当前    regtime2=sysdate,
   HbmOperator.executeSql("update userinfo set regtime5=sysdate where hyid = "+hyinfo.getHyid());
   ...
   //取该秋波匹配的登录时间为半年前的男会员
   //delete by linyu 20150914 for 建松在几年前已经修改了这部分功能。直接取秋波表数据，这个表不再处理
		...
		String sql1 = "select u.hyid from Userinfo u where u.lastTime >= to_date('"+day+"','YYYY-MM-DD') ";
   		...
   		for(int i=0;i<list.size();i++){
        	...
        	if (hyinfo.getSex().equals("10"))
        		sex="她";
	      //  userQiuboMng.sendqiubowarn(hyid, hyinfo.getUsername(),hyinfo.getLcname(),sex, qiubo_number);
   		}
   } */
