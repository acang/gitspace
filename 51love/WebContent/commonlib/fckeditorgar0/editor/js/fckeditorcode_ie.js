...
FCKEvents.prototype.AttachEvent = function(A, B) {
    var C;
    if (!(C = this._RegisteredEvents[A])) this._RegisteredEvents[A] = [B]; else {
...
var FCK = {Name:FCKURLParams['InstanceName'],Status:0,EditMode:0,Toolbar:null,HasFocus:false,DataProcessor:new FCKDataProcessor(),GetInstanceObject:(function() {
    ...
    while (D) {
        var F = false;
        switch (D.nodeType) {case 1:var G = D.nodeName.toLowerCase();if (!FCKListsLib.BlockElements[G] && G != 'li' && !D.getAttribute('_fckfakelement') && D.getAttribute('_moz_dirty') == null) F = true;break;case 3:if (E || D.nodeValue.Trim().length > 0) F = true;break;case 8:if (E) F = true;break;}
        ;
        if (F) {
            var H = D.parentNode;
    ...
    C = C.toUpperCase();
    var D;
    if (!(D = FCK.RegisteredDoubleClickHandlers[C])) FCK.RegisteredDoubleClickHandlers[C] = [A]; else {
    ...
    try {
        ...
        if (!FCK.EditorDocument.queryCommandEnabled(A)) return -1; else {
    ...
    var A;
    if (FCKConfig.FullPage) {
        if (FCK.TempBaseTag.length > 0) A = FCK.TempBaseTag + FCK.GetXHTML(); else A = FCK.GetXHTML();
    } else {
    ...
    if (FCKListsLib.BlockElements[B] != null) {
        if (C.StartBlock) {
            if (C.CheckStartOfBlock()) C.MoveToPosition(C.StartBlock, 3); else if (C.CheckEndOfBlock()) C.MoveToPosition(C.StartBlock, 4); else C.SplitBlock();
        }
        ;
        C.InsertNode(A);
        ...
        if (FCKConfig.AutoDetectPasteFromWord && A.length > 0) {
            ...
            if (B.test(A)) {
                if (confirm(FCKLang.PasteWordConfirm)) {
                    FCKUndo.SaveUndoStep();
                    var T = CleanWord(A, true, true);
                    //var U = T.getElementsByTagName("P");
                    //alert(T);
                    FCK.InsertHtml(T);
                    //FCK.PasteFromWord();
                    return false;
                    ...
                    A = "<style type=\"text/css\">p.MsoNormal{margin:0cm}</style><TABLE style=\"OVERFLOW-X: hidden; WIDTH:630px; WORD-BREAK: break-all\" cellSpacing=0 cellPadding=0 align=center border=0><TBODY><TR><TD>"+A+"</TD></TR></TBODY></TABLE>";
                    //alert(A);
                    FCK.InsertHtml(A);
                    return false;
                }
...
var FCKDomTools = {MoveChildren:function(A, B, C) {
    ...
    if (C) B.insertBefore(FCKDomTools.RemoveNode(A), B.firstChild); else B.appendChild(FCKDomTools.RemoveNode(A));
},TrimNode:function(A) {
    this.LTrimNode(A);
    this.RTrimNode(A);
    ...
    if (B && B.specified) {
        if (C == 'style') return A.style.cssText; else if (C == 'class' || C.indexOf('on') == 0) return B.nodeValue; else {
    ...
    if (FCKBrowserInfo.IsIE) A.removeAttribute(B); else delete A[B];
    ...
    while (true) {
        ...
        if (J.indent == H) {
            ...
            var I;
            if (J.grandparent.nodeName.IEquals(['ul','ol'])) I = D.createElement('li'); else {
                if (FCKConfig.EnterMode.IEquals(['div','p']) && !J.grandparent.nodeName.IEquals('td')) I = D.createElement(FCKConfig.EnterMode); else I = D.createDocumentFragment();
            }
            ;
            for (var i = 0; i < J.contents.length; i++) I.appendChild(J.contents[i].cloneNode(true));
            if (I.nodeType == 11) {
    ...
    if (FCKBrowserInfo.IsIE) return A.currentStyle[B]; else return A.ownerDocument.defaultView.getComputedStyle(A, '').getPropertyValue(B);
...
FCKTools.FixCssUrls = function(A, B) {
    ...
    return B.replace(/url\s*\(([\s'"]*)(.*?)([\s"']*)\)/g, function(match, opener, path, closer) {
        if (/^\/|^\w?:/.test(path)) return match; else return 'url(' + opener + A + path + closer + ')';
    });
};
...
FCKTools.GetVoidUrl = function() {
    ...
    if (FCKBrowserInfo.IsIE) {
        if (FCKBrowserInfo.IsIE7 || !FCKBrowserInfo.IsIE6) return ""; else return "javascript: '';";
    }
    ;
...
FCKTools.CreateXmlObject = function(A) {
    var B;
    switch (A) {case 'XmlHttp':if (document.location.protocol != 'file:') try {
...
FCKXHtml.GetXHTML = function(A, B, C) {
    ...
    if (FCKConfig.DocType.length > 0 && FCKRegexLib.HtmlDocType.test(FCKConfig.DocType)) E = E.replace(FCKRegexLib.SpaceNoClose, '>'); else E = E.replace(FCKRegexLib.SpaceNoClose, ' />');
...
FCKXHtml._AppendAttributes = function(A, B, C, D) {
    ...
    for (var n = 0; n < E.length; n++) {
        ...
        if (F.specified) {
            ...
            } else if (G.indexOf('on') == 0) H = F.nodeValue; else if (D == 'body' && G == 'contenteditable') continue; else if (F.nodeValue === true) H = G; else {
                try {
...
FCKUndo._ApplyUndoLevel = function(A) {
    ...
    if (!B) return;
    if (FCKBrowserInfo.IsIE) {
        if (B[1] && B[1][1]) FCK.SetInnerHtml(B[1][1]); else FCK.SetInnerHtml(B[0]);
...
FCKEditingArea.prototype.Focus = function() {
    try {
        if (this.Mode == 0) {
            if (FCKBrowserInfo.IsIE) this._FocusIE(); else this.Window.focus();
        } else {
...
FCKKeystrokeHandler.prototype.SetKeystrokes = function() {
    for (var i = 0; i < arguments.length; i++) {
        var A = arguments[i];
        if (!A) continue;
        if (typeof(A[0]) == 'object') this.SetKeystrokes.apply(this, A); else {
...
FCKStyle.prototype = {GetType:function() {
    ...
    D = D.replace(/^[ \t]+|[ \t]+$/g, function(match, offset, s) {
        if (match.length == 1) return '&nbsp;'; else if (offset == 0) return new Array(match.length).join('&nbsp;') + ' '; else return ' ' + new Array(match.length).join('&nbsp;');
    });
    ...
    if (FCKBrowserInfo.IsIE) B.outerHTML = '<pre>' + C + '</pre>'; else B.innerHTML = C;
    ...
    while ((F = E.GetNextParagraph())) {
        ...
        var L = J && !K;
        var M = !J && K;
        if (L) I = this._ToPre(G, F, I); else if (M) I = this._FromPre(G, F, I); else FCKDomTools.MoveChildren(F, I);
        ...
        if (e.nodeType == 1) {
            ...
            if (!C) {
                ...
                if (FCKListsLib.PathBlockLimitElements[E] != null) {
                    if (!B && E == 'div' && !FCKElementPath._CheckHasBlock(e)) B = e; else C = e;
                }
            }
            ;
            D.push(e);
...
FCKDomRangeIterator.prototype = {GetNextParagraph:function() {
    ...
    if (D) {
        ...
        if (P && P.nodeType == 1) {
            if (P.nodeName.toLowerCase() == 'br') P.parentNode.removeChild(P); else if (P.lastChild && P.lastChild.nodeName.IEquals('br')) P.removeChild(P.lastChild);
        }
    }
    ;
    if (E) {
...
var FCKEmbedAndObjectProcessor = (function() {
    ...
    return FCKTools.Merge(FCKDocumentProcessor.AppendNew(), {ProcessDocument:function(doc) {
        if (FCKBrowserInfo.IsGecko) FCKTools.RunFunction(H, this, [doc]); else H(doc);
...
FCKSelection.Save = function(A) {
    ...
    if (C) {
        ...
        ;
        if (D) {
            if (D.parentElement && FCKTools.GetElementDocument(D.parentElement()) != B) D = null; else if (D.item && FCKTools.GetElementDocument(D.item(0)) != B) D = null;
        }
    }
    ;
...
FCKTableHandler.VerticalSplitCell = function() {
    ...
    if (G > 1) {
        ...
        if (N.rows.length > L) N.insertBefore(M, N.rows[L]); else N.appendChild(M);
...
FCKDialogCommand.prototype.GetState = function() {
    if (this.GetStateFunction) return this.GetStateFunction(this.GetStateParam); else return FCK.EditMode == 0 ? 0 : -1;
};
...
FCKAnchorDeleteCommand.prototype = {Execute:function() {
    if (FCK.Selection.GetType() == 'Control') {
        ...
        if (A) {
            if (A.tagName == 'IMG' && A.getAttribute('_fckanchor')) oAnchor = FCK.GetRealElement(A); else A = null;
        }
        ;
        if (!A) {
...
FCKPasteWordCommand.prototype.GetState = function() {
    if (FCK.EditMode != 0 || FCKConfig.ForcePasteAsPlainText) return -1; else return FCK.GetNamedCommandState('Paste');
};
...
//    html = html.replaceAll("<P>","<P style=\"line-height: 200%\"><span style=\"font-size: 12pt\"><span style=\"font-family: 楷体_GB2312\">");
//    html = html.replaceAll("</P>","</span></span></P>")
    return html ;
}
