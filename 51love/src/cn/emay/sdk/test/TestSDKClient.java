...
//		Client client2=new Client(softwareSerialNo,password);
	...
	public static void testSendSMS() {
		try {
			int i=SingletonClient.getClient().sendSMS(new String[] {"15010369311"}, "同步内容","",3);//带扩展码
	...
	public static void testsSendScheduledSMS() {
		try {
			int i=SingletonClient.getClient().sendScheduledSMSEx(new String[] {"15000000000"}, "异步内容", "20091104111010","GBK");
	...
	public static void testsSendSMSAddMessageId() {
		try {
			int i=SingletonClient.getClient().sendSMSEx(new String[] {"15010369311"}, "jdk1.4【亿美】", "", "GBK", 5, 1234567);
			System.out.println("testsSendScheduledSMS====="+i);
		} catch (Exception e) {
			e.printStackTrace();
		}
